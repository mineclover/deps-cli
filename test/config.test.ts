...
describe("환경 설정 시스템 테스트", () => {
  ...
  describe("EnvParser", () => {
    test("boolean 파싱", () => {
      expect(EnvParser.parseBoolean("true")).toBe(true)
      expect(EnvParser.parseBoolean("1")).toBe(true)
      expect(EnvParser.parseBoolean("yes")).toBe(true)
      expect(EnvParser.parseBoolean("on")).toBe(true)
      expect(EnvParser.parseBoolean("false")).toBe(false)
      expect(EnvParser.parseBoolean("0")).toBe(false)
      expect(EnvParser.parseBoolean("no")).toBe(false)
      expect(EnvParser.parseBoolean("off")).toBe(false)
      expect(EnvParser.parseBoolean(undefined, true)).toBe(true)
    })

    test("숫자 파싱", () => {
      expect(EnvParser.parseNumber("123")).toBe(123)
      expect(EnvParser.parseNumber("0")).toBe(0)
      expect(EnvParser.parseNumber("invalid", 42)).toBe(42)
      expect(EnvParser.parseNumber(undefined, 42)).toBe(42)
    })

    test("enum 파싱", () => {
      const levels = ['debug', 'info', 'warn', 'error'] as const
      expect(EnvParser.parseEnum("info", levels, "debug")).toBe("info")
      expect(EnvParser.parseEnum("invalid", levels, "debug")).toBe("debug")
      expect(EnvParser.parseEnum(undefined, levels, "debug")).toBe("debug")
    })
  })
