...
const registerCollectDataCommand = (program: Command): void => {
  program
    ...
    .description(
      '🔍 의존성 분석 데이터를 기반으로 네임스페이스별 키워드/파일 경로 데이터를 수집합니다'
    )
...
const registerGeneratePathsCommand = (program: Command): void => {
  program
    ...
    .description(
      '📝 수집된 데이터를 기반으로 네임스페이스별 문서 경로를 생성합니다'
    )
...
const registerCreateCollectionRuleCommand = (program: Command): void => {
  program
    ...
    .option('--document-strategy <strategy>', '문서 생성 전략 (file-mirror|method-mirror|library-structure|namespace-grouping)', 'file-mirror')
    .option('--enable-mirror-tracking', '미러링 추적 활성화')
    .option('--auto-backup', '죽은 파일 자동 백업 활성화')
    .option('--description <desc>', '규칙 설명')
    .action(wrapAction(async (namespace, options) => {
      try {
...
const registerUpdateCollectionRuleCommand = (program: Command): void => {
  program
    ...
    .option('--document-strategy <strategy>', '문서 생성 전략 (file-mirror|method-mirror|library-structure|namespace-grouping)')
...
async function outputResults(results: any[], options: any) {
  const output = {
    ...
    results: options.format === 'summary'
      ? results.map(r => ({
  ...
  program
    ...
    .action(wrapAction(async (options) => {
      ...
      for (const rule of rules) {
        ...
        const result = await manager.collectForNamespace(
          dependencyGraph,
          rule,
          executionConfig,
          moduleOptions
...
const registerUpdateAllCommand = (program: Command): void => {
  program
    .command('update-all')
    .description(
      '🚀 모든 네임스페이스에 대해 데이터 수집, 경로 생성 및 문서 생성을 일괄 실행합니다'
    )
    .option('--config <file>', '수집 규칙 설정 파일 경로', 'deps-cli.json')
    .option('--target <path>', '분석할 대상 경로', '.')
