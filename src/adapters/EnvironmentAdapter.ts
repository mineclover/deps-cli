...
export class EnvParser {
   ...
   * 문자열을 숫자로 변환
   */
  static parseNumber(value: string | undefined, defaultValue: number = 0): number {
    if (!value) return defaultValue
   ...
   * 환경 변수 값의 유효성 검증
   */
  static validate(value: string | undefined, validator: (val: string) => boolean): boolean {
...
export class EnvironmentAdapter implements ConfigAdapter {
  ...
  async load(): Promise<Partial<EnvironmentConfig>> {
    ...
    // 분석 설정
    config.analysis = {
      maxConcurrency: this.parseNumberEnvVar(
        'DEPS_CLI_MAX_CONCURRENCY',
        this.env.DEPS_CLI_MAX_CONCURRENCY,
        4,
        { min: 1, max: 100 }
      ),
      timeout: this.parseNumberEnvVar(
        'DEPS_CLI_TIMEOUT',
        this.env.DEPS_CLI_TIMEOUT,
        30000,
        { min: 1000, max: 300000 }
      ),
      cacheEnabled: this.parseBooleanEnvVar(
        'DEPS_CLI_CACHE_ENABLED',
        this.env.DEPS_CLI_CACHE_ENABLED,
        true
      ),
    ...
    config.development = {
      verbose: this.parseBooleanEnvVar(
        'DEPS_CLI_VERBOSE',
        this.env.DEPS_CLI_VERBOSE,
        isDevelopment
      ),
      debugMode: this.parseBooleanEnvVar(
        'DEPS_CLI_DEBUG',
        this.env.DEPS_CLI_DEBUG || this.env.DEBUG,
        isDevelopment
      ),
      mockApiCalls: this.parseBooleanEnvVar(
        'DEPS_CLI_MOCK_API',
        this.env.DEPS_CLI_MOCK_API,
        false
      )
    }
  ...
  private parseNumberEnvVar(
    ...
    const result = EnvParser.parseNumber(value, defaultValue)
    let isValid = true
  ...
  private parseBooleanEnvVar(key: string, value: string | undefined, defaultValue: boolean): boolean {
    const result = EnvParser.parseBoolean(value, defaultValue)
    this.setMetadata(key, {
      source: 'env',
      raw: value,
      parsed: result,
